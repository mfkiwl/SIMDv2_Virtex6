...
-- Date:              Wed Jul 23 16:10:06 2014 (by Create and Import Peripheral Wizard)
...
use simdv2_v1_00_a.simdv2_glue;
...
architecture IMP of user_logic is
  ...
  --USER signal declarations added here, as needed for user logic
	...
	signal T_reg, T_mem_w, T_mem_r: std_logic;	--***Trigger signals***--
...
begin
  ...
  --USER logic implementation added here
	...
	----****---Instatiating the SIMD Implementation---***---
	ut: entity simdv2_v1_00_a.simdv2_glue
		port map
		(
			clk 	=> Bus2IP_Clk,
          		rst 	=> Bus2IP_Resetn,
  ...
  begin
    ...
    if Bus2IP_Clk'event and Bus2IP_Clk = '1' then
      if Bus2IP_Resetn = '0' then
		...
		T_reg	<='1';	--Read all the registers from all PE's in to slv_reg1,2,3,4
              end if;
            end loop;
          ...
          when "0000010000" =>
            for byte_index in 0 to (C_SLV_DWIDTH/8)-1 loop	--*****5: memory write address--*****
              if ( Bus2IP_BE(byte_index) = '1' ) then
                	slv_reg5(byte_index*8+7 downto byte_index*8) <= Bus2IP_Data(byte_index*8+7 downto byte_index*8);
			T_mem_w<='1';	--Trigger PE memory write  
              end if;
            end loop;
          when "0000001000" =>
            for byte_index in 0 to (C_SLV_DWIDTH/8)-1 loop	--*****6: memory write Data--*****
              if ( Bus2IP_BE(byte_index) = '1' ) then
			--Store data for memory WRITE operation
                	slv_reg6(byte_index*8+7 downto byte_index*8) <= Bus2IP_Data(byte_index*8+7 downto byte_index*8);
              end if;
            end loop;
          when "0000000100" =>
            for byte_index in 0 to (C_SLV_DWIDTH/8)-1 loop	--*****7: memory Read address--*****
              if ( Bus2IP_BE(byte_index) = '1' ) then
                	--Store address for memory READ operation	
                	slv_reg7(byte_index*8+7 downto byte_index*8) <= Bus2IP_Data(byte_index*8+7 downto byte_index*8);
			T_mem_w<='1';	--Trigger PE memory read
              end if;
            end loop;
          when "0000000010" =>
            for byte_index in 0 to (C_SLV_DWIDTH/8)-1 loop
